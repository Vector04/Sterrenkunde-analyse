...
def log_interpolate(y0, y1, num_values):
    ...
    if num_values <= 0:
        raise ValueError(
            'Invalid number of divisions %s in log_interpolate' % (str(num_values)))
    if num_values == 1:
...
def xyz_patch_plot(
    ...
    rgb_patch_plot(rgb_colors, color_names, title, filename,
...
def shark_fin_plot():
    ...
    xyz_list = ciexyz.get_normalized_spectral_line_colors(
